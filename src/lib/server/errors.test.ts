...
describe('AppError', () => {
	describe('基本プロパティ', () => {
		it('code, status, message が正しく設定される', () => {
		...
		it('fields を指定しない場合は undefined になる', () => {
			const err = new AppError('NOT_FOUND', 404, '該当データが見つかりません');
			expect(err.fields).toBeUndefined();
		});

		it('fields を指定した場合に正しく設定される', () => {
		...
		it('instanceof AppError でキャッチできる', () => {
			expect(() => {
	...
	describe('エラーコード', () => {
		...
		it('INTERNAL_SERVER_ERROR を設定できる', () => {
			const err = new AppError('INTERNAL_SERVER_ERROR', 500, 'サーバーエラーが発生しました');
