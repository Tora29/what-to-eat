...
describe('Button', () => {
	describe('size prop', () => {
		test('[SPEC: AC-001] size="sm" で py-1.5 text-xs が付与される', async () => {
			...
			const el = page.getByRole('button', { name: 'Click' });
	...
	describe('variant prop', () => {
		test('[SPEC: AC-004] variant="primary" で bg-accent が付与される', async () => {
			render(Button, {
				variant: 'primary',
				children: makeLabel('Click')
			});
		...
		test('[SPEC: AC-005] variant="secondary" で border-separator が付与される', async () => {
			render(Button, {
				variant: 'secondary',
				children: makeLabel('Click')
			});
		...
		test('[SPEC: AC-006] variant="destructive" で bg-destructive が付与される', async () => {
			render(Button, {
				variant: 'destructive',
				children: makeLabel('Click')
			});
		...
		test('[SPEC: AC-007] variant="ghost-destructive" で bg-destructive/10 が付与される', async () => {
			render(Button, {
	...
	describe('disabled prop', () => {
		test('[SPEC: AC-008] disabled 時に disabled 属性が付与される', async () => {
			render(Button, {
				disabled: true,
				children: makeLabel('Click')
			});
	...
	describe('children', () => {
		test('[SPEC: AC-009] children が描画される', async () => {
			render(Button, {
				children: makeLabel('ボタンテキスト')
			});
			const el = page.getByRole('button', { name: 'ボタンテキスト' });
